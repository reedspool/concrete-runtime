...
Tape.containsEnd = function(tape) {
  return tape.get('blocks').filter(function (block) {
    return Block.isEnd(block)
  }).size > 0;
}
...
Tape.getBlock = function(location) {
  ...
  if ( ! location.get('tape').getIn(['blocks', index]) ) throw new Error('Location non-existant');
